# Conversion UX Model - Beaver Study Orchestrator
...
| Primary buyer or user | Students, advisors, bootcamps, and education platforms. |
| Value wedge | Syllabus-to-schedule planner with interpretable risk and what-if simulation. |
...
| First action | Open the portfolio-fit note, confirm the successor project, then inspect make test passes only if this domain is relevant. |
...
- Use Schedule completion only as a discussion metric until fresh validation exists.
